...
from app.pipeline.extract import extract_excel
from app.pipeline.load import load_em_um_novo_excel
...
def mock_input_folder(tmpdir):
    ...
    df1.to_excel(os.path.join(input_folder, "file1.xlsx"), index=False)
    df2.to_excel(os.path.join(input_folder, "file2.xlsx"), index=False)
...
def test_load(mock_output_folder):
    ...
    # Verifying the existence of the output file
    assert os.path.exists(os.path.join(mock_output_folder, output_file_name))

    # Verifying the contents of the loaded file
    loaded_df = pd.read_excel(os.path.join(mock_output_folder, output_file_name))
